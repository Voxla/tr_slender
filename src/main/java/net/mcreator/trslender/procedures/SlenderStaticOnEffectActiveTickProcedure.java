...
public class SlenderStaticOnEffectActiveTickProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((entity.getCapability(TrSlenderModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new TrSlenderModVariables.PlayerVariables())).staticnorm == 450) {
			if (Math.random() > 0.5) {
		...
		if (!world.getEntitiesOfClass(SlendermanEntity.class, AABB.ofSize(new Vec3(x, y, z), 50, 50, 50), e -> true).isEmpty()) {
			if (new Object() {
				...
				if (entity instanceof LivingEntity _livEnt11 && _livEnt11.hasEffect(TrSlenderModMobEffects.SLENDER_STATIC.get())) {
